...
app=Flask(__name__)
import pickle
model=pickle.load(open(r'C:/Users/Nithin R/Downloads/ML_Project2/cerealanalysis.pkl','rb'))
@app.route('/') 
def helloworld():
...
def admin():
    ...
    n=request.form["Cups"]
    z=int(float(n))
    t=[[int(a1),int(a2),int(a3),int(a4),int(a5),int(a6),int(a7),int(b),int(c),int(d),int(e),int(f),int(g),int(h),int(i),int(j),int(k),int(l),int(m),z]]
    print(t)
    y=model.predict(t)
